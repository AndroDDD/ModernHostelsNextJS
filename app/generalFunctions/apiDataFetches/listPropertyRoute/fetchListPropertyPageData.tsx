...
const generateListPropertyPageData = (data: {
  ...
  if (data.how_it_works_section.sub_sections) {
  ...
  const generatedListPropertyPageData: ListPropertyPageData = {
    introSection: {
      title: data.intro_section.title,
      subTitle: data.intro_section.sub_title,
      statement: data.intro_section.statement,
      backgroundImageUrl: data.intro_section.background_img_url,
    },

    earningsSection: {
      title: data.earnings_section.title,
      statement: data.earnings_section.statement,
      statsImage: data.earnings_section.img_url,
      stats: data.earnings_section.stats,
    },

    slidingInformationSection: {
      title: data.sliding_information_section.title,
      statement: data.sliding_information_section.statement,
      info: data.sliding_information_section.slides.map((slide) => ({
        title: slide.title,
    ...
    clienteleSection: {
      title: data.clientele_section.title,
      statement: data.clientele_section.statement,
      imgUrl: data.clientele_section.img_url,
      benefits: data.clientele_section.client_types,
    },

    propertiesSlideSection: {
      properties: data.properties_slide_section.map((slide) => ({
        name: slide.name,
    ...
    singleReviewSection: {
      from: data.single_review_section.reviewer_name,
      statement: data.single_review_section.reviewer_comment,
      backgroundImage: data.single_review_section.background_img_url,
    },

    howItWorksSection: {
      title: data.how_it_works_section.title,
      subTitle: data.how_it_works_section.sub_title,
      steps: formattedHowItWorksSectionSteps,
    },

    slidingImagesSection: {
      title: data.sliding_images_section.title,
      statement: data.sliding_images_section.statement,
      images: Object.values(data.sliding_images_section.images),
    },

    faqsSection: {
      title: data.faqs_section.title,
      inquiries: data.faqs_section.faqs,
    },
  };
